...
`
      : "";
    const designScopeConstraints = params.designRequirements
      ? `## スコープ制約（requirements 原文）
${params.designRequirements}

- requirements で「スコープ外」「不要」と明示された機能は、仕様・テストケース・実装へ追加してはならない
- 「未定義」の指摘に対し、該当機能がスコープ外なら、定義を足す代わりに「スコープ外」と明記して除外する修正も有効
...
${retrySummary}${designScopeConstraints}## 前提
...
- 指摘が「未定義」を指す場合、該当機能が requirements 上でスコープ外なら、「仕様へ追加する」のではなく「仕様書から除外し、スコープ外セクションへ明記する」方針を選べる
...
`
      : "";
    const designScopeConstraints = params.designRequirements
      ? `## Scope Constraints (requirements 原文)
${params.designRequirements}

- requirements で「スコープ外」「不要」と明示された機能は追加しない
- 未定義指摘に対して、scope 外なら定義追加ではなく「除外明記」または関連記述の削除で対応してよい
...
${retrySummary}${designScopeConstraints}## 実行ルール
- 修正根拠は上の修正計画だけに限定する
