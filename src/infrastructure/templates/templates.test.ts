...
test("`loadTemplate` は bundled template へフォールバックし、未知名は例外にする", () => {
  ...
  assert.match(reviewSpecTcTemplate, /## requirements/);
  assert.match(reviewSpecTcTemplate, /スコープ外 \/ 不要と明示/);
  ...
  assert.match(reviewSpecTemplate, /## requirements/);
  assert.match(reviewSpecTemplate, /6\. スコープ整合性/);
