...
1. Codex / Claude から `harness-pilot` や project-local skill を直接使うなら、先に `sync-skills` を実行する。
2. `.harness/config/harness.example.yml` を `.harness/config/harness.yml` にコピーする。
3. 最初に使う profile を決める。通常は `backend` か `frontend` のどちらか。
4. 選んだ profile に必要な lint / test / runners が揃っていることを確認する。
5. 実行結果は `.harness/logs/` と `.harness/reviews/` に出ると理解してからコマンドを選ぶ。
