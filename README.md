...
ローカル LLM にこのハーネスを扱わせるときは、まず `harness-pilot` skill を読み込ませる前提です。
`harness-pilot` は最初にこの README を正本として読み、必要なときだけ追加ドキュメントを参照します。
...
1. Codex / Claude から `harness-pilot` や project-local skill を直接使う場合は、`.harness/resources/skills/` を `.codex/skills/` / `.claude/skills/` に反映する。`./.harness/bin/harness sync-skills` を使うか、手動でコピーする。
2. `.harness/config/harness.example.yml` を `.harness/config/harness.yml` にコピーする。
3. 最初に使う profile を決める。現時点では `backend` を基準とし、`frontend` フローは TBD / 開発中として扱う。
4. 選んだ profile に必要な lint / test / runners が揃っていることを確認する。
5. 実行ログは `.harness/logs/`、最終レポートは `.harness/reviews/` に出ることを把握する。

`sync-skills` は、Codex / Claude などのローカル LLM が `harness-pilot` や project-local skill を直接呼び出す運用で必要です。
ハーネス内部実行では `.harness/resources/skills/` を正本として直接参照できます。

- Codex 系 runner / Codex ローカル LLM を使う場合: `.codex/skills/`
- Claude 系 runner / Claude Code を使う場合: `.claude/skills/`

通常は `sync-skills` を実行すれば、両方の managed copy をまとめて同期できます:
```bash
...
- 実行ログ: `.harness/logs/{timestamp}_{task_name}/`
- イベント時系列: `harness.jsonl`
- レビュー構造化データ: `review-data.json`
- `--resume` 用チェックポイント: `checkpoint.json`
- 最終レポート: `.harness/reviews/{date}_{scope}.md`
- 比較・診断: `./.harness/bin/harness benchmark-summary <log-dir> [<log-dir>]`, `./.harness/bin/harness benchmark-diagnose <log-dir> [<log-dir>]`

失敗時は、まず `harness.jsonl`、次に `review-data.json`、再開系なら `checkpoint.json` を確認します。ログ構造の詳細は `logs/README.md` を参照してください。
...
- LLM 文脈: `context.defaultAgent`, `context.defaultSkills`, `context.stepOverrides`
...
- `design` では `--profile <name>` を付ける
- `impl` / `page` / `component` では plan frontmatter の `profile:` で指定する
- profile が 1 つだけなら省略可能
- 最初は単一 profile から始め、必要になってから分割する方が運用しやすい
...
runner の切り替えは `profiles.<name>.steps` で行います。`runners.<name>.model` は runner のデフォルト、`profile.context.stepOverrides.<step>.model` はその step 専用の上書きです。

`generic` runner の主要フィールド:

| フィールド | 必須 | 説明 |
|---|---|---|
| command | o | 実行する CLI コマンド |
| args | o | コマンドの固定引数 |
| promptFlag | - | プロンプトを渡すフラグ。未指定時は stdin に流す |
...
step 実行時の skill 探索順は `.codex/skills/<name>/SKILL.md` → `.harness/resources/skills/<name>/SKILL.md` → `.claude/skills/<name>/SKILL.md` です。repo 固有 override が不要なら `.harness/resources/skills/` を正本として扱えます。
...
runtime skill の正本は `.harness/resources/skills/` に置きます。ハーネス内部実行はここを直接参照できます。ローカル LLM が `harness-pilot` などを直接読む運用では、使うサービスに応じて `.codex/skills/` または `.claude/skills/` に managed copy が必要です。通常は `./.harness/bin/harness sync-skills` を実行して両方を同期します。
...
- `codex-review.log` — generic / codex コマンド実行ログ
